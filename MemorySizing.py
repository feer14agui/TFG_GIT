...
def porc(x,y): #función para obtener el porcentaje
    result = (x * 100.0) / (y)
    return result

def XML (): #función para crear el fichero XML
...
def AddDicc(types): #función encargada de crear el diccionario general
...
for i in sys.argv:
    infile = open(i, 'r') #variable en la cual encontraremos el fichero abierto

    memory = 0 #se inicializa la memoria total

    i=0

    module = {} #diccionario general
    modmemory = {} #diccionario de los módulos
    typesmemory = {} #diccionario de los tipos de datos
    expresion = r'(\.\w{3,15}\s{0,10}0x\w{5,20}\s{3,10}0x\w{2,5} .{0,100})'
    for line in infile:
        patron = re.search(expresion, line) #variable para comprobar si se encuentra la expresión
        if patron:
